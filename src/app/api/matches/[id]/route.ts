...
// Ensure Node runtime (service role + admin auth won't work on Edge)
      ...
      if (hostAuthUser) {
        hostProfile = {
          ...
          full_name:
            ...
            (hostAuthUser.user_metadata as Record<string, unknown>)?.['full_name'] as string | null ??
            null,
          discord_username:
            ((hostAuthUser.user_metadata as Record<string, unknown>)?.['discord_username'] as string) ??
            ((hostAuthUser.user_metadata as Record<string, unknown>)?.['name'] as string) ??
      ...
      if (opponentAuthUser) {
        opponentProfile = {
          ...
          full_name:
            ...
            (opponentAuthUser.user_metadata as Record<string, unknown>)?.['full_name'] as string | null ??
            null,
          discord_username:
            ((opponentAuthUser.user_metadata as Record<string, unknown>)?.['full_name'] as string) ??
            ((opponentAuthUser.user_metadata as Record<string, unknown>)?.['name'] as string) ??
...
export async function DELETE(request: NextRequest, { params }: RouteCtx) {
  ...
  try {
    ...
    if (match.user_id !== user.id) {
      return NextResponse.json({ error: 'Unauthorized to cancel this match' }, { status: 403 });
    }
    ...
    if (refundError) {
      ...
      return NextResponse.json({ error: 'Failed to refund entry fee' }, { status: 500 });
    }
